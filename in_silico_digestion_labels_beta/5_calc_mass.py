...
def inlines_MS1(input_file):
    """
    Read the input file for MS1 (output.4.MS1)
    """
    input_lines = []
...
def lines_final(input_lines, mzlow, mzhigh, nts_light=nts_mass(read_excel_input(args.nts_light))[0]):
    ...
    for line in input_lines:
        ...
        elif line.split()[8] == "App":
            l_mass, h_mass = l_mass + nts_light["A"]  + ele_mass["P"] + ele_mass["O"] * 4 + ele_mass[
                "H"] * 2, h_mass + nts_heavy["A"]  + ele_mass["P"] + ele_mass["O"] * 4 + ele_mass["H"] * 2
...
if os.path.exists(os.getcwd() + "/output.4.MS1"):

    charge_table(os.getcwd() + "/" + args.MS1_charges)
    # Write all the lines inside the standard output file "Digest_MS1"
    open(os.getcwd() + "/Digest_MS1.txt", 'w').writelines(header_info_MS1(open(os.getcwd() + "/output.4.MS1", 'r')) +
                                                          lines_final(lines_with_charge(lines_with_chemistry(
                                                              inlines_MS1(open(os.getcwd() + "/output.4.MS1", 'r')))),
                                                              args.MS1_mzlow, args.MS1_mzhigh))
    out_files.append("Digest_MS1.txt")

else:
    print("WARNING! MS1 file output.4.MS1 from 3_consolidate.py is missing, MS1 digest will not be generated")

################################# MS2 SPECIFIC WORKFLOW #############################################
"""
This part is executed only under the condition that the file output.4.MS2 is present in the directory
"""

# Checking if the input file output.4.MS2 is present
if os.path.exists(os.getcwd() + "/output.4.MS2"):

    def inlines_MS2(input_file=open(os.getcwd() + "/output.4.MS2", 'r')):
        """
        ...
        for b in unique_seq:
            ...
            if line.split()[1] == "light":
                ...
                if chem3prime == 'P' or chem5prime == 'P':
                    ...
                    # Add the special case of base double loss (with one being the 3'end base) for cP 3' chemistry
                    if b in line.split()[7][:-1]:
                        MBHBH_mz = np.float64(line.split()[0]) - (nts_dic_onlyB_light[b] + ele_mass["H"] +
                ...
                if chem3prime == 'P' or chem5prime == 'P':
                    ...
                    # Add the special case of base double loss (with one being the 3'end base) for cP 3' chemistry
                    if b in line.split()[7][:-1]:
                        MBHBHh_mz = np.float64(line.split()[0]) - (nts_dic_onlyB_heavy[b] + ele_mass["H"] +
    ...
    def final_lines_MS2(lines_MS2):
        ...
        global tot_decoys
        tot_decoys = 0
        global tot_tagged_targets
        tot_tagged_targets = 0
    ...
    final_header = (header_info_MS2(open(os.getcwd() + "/output.4.MS2", 'r')) +
                    ["#TARGETS {}\n#TAGGED {}\n#DECOYS {}\n".format(tot_targets, tot_tagged_targets, tot_decoys)] +
    ...
    print("WARNING! MS2 file output.4.MS2 from 3_consolidate.py is missing, MS2 digest will not be calculated")
